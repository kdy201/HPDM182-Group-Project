...
df_classified <- df_classified %>%
  mutate(
    income_class_num = case_when(
      income_class == "Low"  ~ 0,
      income_class == "Mid"  ~ 1,
      income_class == "Upper-middle" ~ 2,
      income_class == "High" ~ 3
    )
  )
)
...
df_gdp <- df_classified %>%
  select(geo, name, avg_income, income_class_num)
df_gdp
...
#Linear Model: Cases
lm_cases <- lm(avg_cases ~ avg_income* income_class_num, data=df_merged)
lm_cases %>% summary()
